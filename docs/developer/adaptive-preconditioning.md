...
Room Assist stores additional per-climate fields in the same backend settings object. Those fields and their runtime behavior are documented in [Room Assist internals](room-assist.md).
...
- `preconditioning_diagnostics`, a compact calculation breakdown for the authoritative plan;
- `weekday` and `start`.
...
- `comfort_percentile`;
- `complete_rate_minutes_per_degree` when the history model is active;
- `complete_estimate_minutes`;
- `partial_floor_minutes`;
- `combined_estimate_minutes`;
- `rounded_estimate_minutes`;
- `final_lead_minutes`;
- `limited_by_min_start` and `limited_by_max_lead`;
...
```text
increment = max(1, ceil(startup_minutes * 0.20))
floor = startup_minutes + increment
```

The partial increment is a percentage of elapsed time, not a fixed temperature or degree-based value. This keeps the lower-bound behavior consistent for Celsius and Fahrenheit installations and avoids overcorrecting very short attempts or undercorrecting very long attempts. If several selected partial samples are still active, Velair computes this floor for each of them and uses the largest floor. The most recent partial does not automatically win; recency only determines expiry and whether later complete samples have refuted that partial.
...
```text
P1 floor would be 90 + ceil(90 * 0.20) = 108
...
J floor would be 100 + ceil(100 * 0.20) = 120
...
Velair can close a session early only when the effective room temperature source emits a state update. By default that source is the climate entity's `current_temperature`; when Room Sensor Assist is enabled with a selected `room_temperature_entity_id`, it is the selected sensor. If the active source does not publish temperature changes during the preconditioning window, Velair evaluates the result at the scheduled comfort time.
