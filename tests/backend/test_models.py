...
class ScheduleBlockNormalizationTest(unittest.TestCase):
    ...
    def test_normalize_blocks_preserves_optional_climate_settings(self) -> None:
        blocks = normalize_schedule_blocks(
            [
                {
                    "start": "22:00",
    ...
    def test_normalize_preconditioning_data_preserves_room_sensor_assist_settings(
        self,
    ) -> None:
        data = normalize_preconditioning_data(
            {
        ...
        self.assertEqual(
            data["room_temperature_entity_id"],
    ...
    def test_normalize_preconditioning_data_caps_room_sensor_assist_max_delta(
        ...
        data = normalize_preconditioning_data(
            {
                "room_sensor_assist_max_delta": 20,
            }
        )

        self.assertEqual(data["room_sensor_assist_max_delta"], 18.0)

    def test_normalize_preconditioning_data_caps_room_sensor_assist_debounce(
        self,
    ) -> None:
        data = normalize_preconditioning_data(
            {
                "room_sensor_assist_debounce_seconds": 500,
            }
        )

        self.assertEqual(data["room_sensor_assist_debounce_seconds"], 300)

    def test_normalize_preconditioning_data_allows_short_room_sensor_assist_debounce(
        self,
    ) -> None:
        data = normalize_preconditioning_data(
            {
    ...
    def test_normalize_preconditioning_data_allows_zero_room_sensor_assist_debounce(
        self,
    ) -> None:
        data = normalize_preconditioning_data(
            {
        ...
        self.assertEqual(data["room_sensor_assist_debounce_seconds"], 0)
    ...
    def test_adaptive_prediction_uses_initial_model_without_complete_samples(self) -> None:
        ...
        self.assertEqual(prediction["diagnostics"]["direction"], "heat")
        self.assertEqual(prediction["diagnostics"]["delta_temperature"], 2)
        self.assertEqual(prediction["diagnostics"]["complete_estimate_minutes"], 80)
        self.assertEqual(prediction["diagnostics"]["partial_floor_minutes"], 0)
        self.assertEqual(prediction["diagnostics"]["combined_estimate_minutes"], 80)
        self.assertEqual(prediction["diagnostics"]["rounded_estimate_minutes"], 80)
        self.assertEqual(prediction["diagnostics"]["final_lead_minutes"], 80)
    ...
    def test_adaptive_prediction_uses_weighted_percentile_after_five_complete_samples(self) -> None:
        ...
        self.assertIsNotNone(prediction["diagnostics"])
        assert prediction["diagnostics"] is not None
        self.assertEqual(prediction["diagnostics"]["complete_sample_count"], 5)
        self.assertEqual(prediction["diagnostics"]["partial_sample_count"], 0)
        self.assertEqual(prediction["diagnostics"]["complete_estimate_minutes"], 70)
        self.assertEqual(prediction["diagnostics"]["partial_floor_minutes"], 0)
        self.assertEqual(prediction["diagnostics"]["combined_estimate_minutes"], 70)
        self.assertEqual(prediction["diagnostics"]["final_lead_minutes"], 70)
    ...
    def test_adaptive_prediction_rounds_partial_floor_up_to_next_five_minutes(
        ...
        assert prediction["diagnostics"] is not None
        self.assertEqual(prediction["diagnostics"]["complete_estimate_minutes"], 70)
        self.assertEqual(prediction["diagnostics"]["partial_floor_minutes"], 96)
        self.assertEqual(prediction["diagnostics"]["combined_estimate_minutes"], 96)
        self.assertEqual(prediction["diagnostics"]["rounded_estimate_minutes"], 100)

    def test_adaptive_prediction_partial_floor_uses_percentage_without_fixed_minimum(
        self,
    ) -> None:
        config = normalize_preconditioning_data(
            {"adaptive_percentile_enabled": False}
        ...
        observations = [
            *[
                _preconditioning_observation(10, quality="complete", delta_t=2)
                for _ in range(5)
            ],
            _preconditioning_observation(20, quality="partial", delta_t=2),
        ]
        ...
        self.assertEqual(prediction["source"], "history")
        self.assertEqual(prediction["partial_sample_count"], 1)
        self.assertEqual(prediction["recommended_lead_minutes"], 25)

    def test_adaptive_prediction_partial_floor_uses_percentage_without_fixed_maximum(
        self,
    ) -> None:
        config = normalize_preconditioning_data(
            {"adaptive_percentile_enabled": False}
        ...
        observations = [
            *[
            ...
            _preconditioning_observation(
                ...
                quality="partial",
                delta_t=2,
                created_at="2026-05-19T19:59:00+00:00",
            ),
        ]
        ...
        self.assertEqual(prediction["source"], "history")
        self.assertEqual(prediction["partial_sample_count"], 1)
        self.assertEqual(prediction["recommended_lead_minutes"], 360)

    def test_adaptive_prediction_partial_floor_is_temperature_unit_agnostic(
        self,
    ) -> None:
        config = normalize_preconditioning_data(
            {"adaptive_percentile_enabled": False}
        )
        celsius_observations = [
            *[
                _preconditioning_observation(10, quality="complete", delta_t=2)
                for _ in range(5)
            ],
        ...
        celsius_prediction = predict_preconditioning_lead(
            celsius_observations,
            "heat",
            target_temp=21,
            current_temp=19,
        ...
        fahrenheit_prediction = predict_preconditioning_lead(
            fahrenheit_observations,
            "heat",
            target_temp=69.8,
            current_temp=66.2,
        ...
        self.assertEqual(celsius_prediction["recommended_lead_minutes"], 110)
        self.assertEqual(fahrenheit_prediction["recommended_lead_minutes"], 110)
