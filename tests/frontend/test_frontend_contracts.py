...
FRONTEND_SENSORS_STYLES_SOURCE = ROOT / "frontend" / "src" / "velair" / "styles" / "sensors-styles.ts"
...
FRONTEND_SENSORS_VIEW_SOURCE = ROOT / "frontend" / "src" / "velair" / "views" / "sensors-view.ts"
...
class FrontendSourceContractTest(unittest.TestCase):
    ...
    def test_room_sensor_assist_services_have_managed_climate_selectors(self) -> None:
        """Room Sensor Assist automation services must target one climate entity."""
        services_source = SERVICES_YAML_SOURCE.read_text(encoding="utf-8")

        for service_name in (
            "enable_room_sensor_assist",
            "disable_room_sensor_assist",
        ):
            self.assertIn(f"{service_name}:", services_source)
            service_block = services_source.split(f"{service_name}:", 1)[1].split(
                "\n\n",
                1,
            )[0]
            self.assertIn("entity_id:", service_block)
            self.assertIn("domain: climate", service_block)
    ...
    def test_frontend_has_modular_foundation_files(self) -> None:
        ...
        sensors_styles_source = FRONTEND_SENSORS_STYLES_SOURCE.read_text(encoding="utf-8")
        ...
        self.assertIn("export const sensorsStyles = css`", sensors_styles_source)
    ...
    def test_sensors_has_a_dedicated_panel_view(self) -> None:
        """Room sensor controls should be separate from Adaptive Preconditioning."""
        constants_source = FRONTEND_CONSTANTS_SOURCE.read_text(encoding="utf-8")
        content_source = FRONTEND_CARD_CONTENT_SOURCE.read_text(encoding="utf-8")
        preconditioning_source = FRONTEND_PRECONDITIONING_VIEW_SOURCE.read_text(encoding="utf-8")
        sensors_source = FRONTEND_SENSORS_VIEW_SOURCE.read_text(encoding="utf-8")
        tabs_source = FRONTEND_TABS_SOURCE.read_text(encoding="utf-8")

        self.assertIn('"sensors"', constants_source)
        self.assertIn('view: "sensors"', tabs_source)
    ...
    def test_card_renders_from_host_view_attribute(self) -> None:
        ...
        self.assertIn('if (view === "sensors")', source)
    ...
    def test_overview_view_has_status_events_and_zone_metrics(self) -> None:
        ...
        self.assertIn("formatDateTime(value, host._dateLocale(), host.hass?.locale?.time_format)", source)
        self.assertIn("export function formatDateTime", formatters_domain_source)
        self.assertIn("export function formatScheduleTime", formatters_domain_source)
